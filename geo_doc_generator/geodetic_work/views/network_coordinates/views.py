...
class NetworkCoordinatesCreateView(CreateView, LoginRequiredMixin, UserPassesTestMixin):
    model = GeodeticNetworkCoordinates
    ...
    def form_valid(self, form):
        ...
        form.save()
        messages.success(
            self.request, "Dodałeś dokument: Wykaz współrzędnych osnowy pomiarowej"
        )
...
class NetworkCooridnatesUpdateView(UpdateView, LoginRequiredMixin, UserPassesTestMixin):
    model = GeodeticNetworkCoordinates
    template_name = "geodetic_work/document_create.html"
    fields = "__all__"
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            "Zmieniłeś dane dokumentu: Wykaz współrzędnych osnowy pomiarowej",
        )
        return redirect("geodetic-work-details", work_id)
