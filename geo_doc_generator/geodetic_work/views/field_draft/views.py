...
class FieldDraftCreateView(CreateView, LoginRequiredMixin, UserPassesTestMixin):
    model = FieldDraft
    ...
    def form_valid(self, form):
        ...
        form.save()
        messages.success(self.request, "Dodałeś dokument: Szkic pomiarowy")
...
class FieldDraftUpdateView(UpdateView, LoginRequiredMixin, UserPassesTestMixin):
    model = FieldDraft
    template_name = "geodetic_work/document_create.html"
    fields = "__all__"
    ...
    def form_valid(self, form):
        work_id = form.data.get("id_work")
        form.save()
        messages.success(self.request, "Zmieniłeś dane dokumentu: Szkic pomiarowy")
        return redirect("geodetic-work-details", work_id)
