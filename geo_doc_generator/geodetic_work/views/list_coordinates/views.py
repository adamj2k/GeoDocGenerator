...
class ListCoordinatesCreateView(CreateView, LoginRequiredMixin, UserPassesTestMixin):
    model = ListOfCoordinates
    ...
    def form_valid(self, form):
        ...
        form.save()
        messages.success(
            self.request, "Dodałeś dokument: Wykaz współrzędnych punktów pomierzonych"
        )
...
class ListCoordinatesUpdateView(UpdateView, LoginRequiredMixin, UserPassesTestMixin):
    model = ListOfCoordinates
    template_name = "geodetic_work/document_create.html"
    fields = "__all__"
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            "Zmieniłeś dane dokumentu: Wykaz współrzędnych punktów pomierzonych",
        )
        return redirect("geodetic-work-details", work_id)
