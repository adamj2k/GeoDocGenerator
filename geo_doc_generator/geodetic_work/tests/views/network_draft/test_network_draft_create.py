...
class TestGeodeticNetworkDraftCreateView(TestCase):
    def setUp(self) -> None:
        super().setUp()
        self.data = {"id_work": self.test_geodetic_work1.id, "pdf_file": ""}
    ...
    def test_should_return_status_code_200_when_request_is_sent(self):
        ...
        response = client.get(
            reverse("network-draft-create", kwargs={"pk": self.test_geodetic_work1.pk})
        )
        actual_code = response.status_code

        self.assertEqual(expected_code, actual_code)

    def test_should_create_new_object_geodetic_network_draft(self):
        ...
        self.assertEqual(
            GeodeticNetworkDraft.objects.filter(
