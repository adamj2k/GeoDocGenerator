...
class TestGeodeticNetworkSurveyCreateView(TestCase):
    def setUp(self) -> None:
        super().setUp()
        self.data = {
    ...
    def test_should_return_status_code_200_when_request_is_sent(self):
        ...
        response = client.get(
            reverse("network-survey-create", kwargs={"pk": self.test_geodetic_work1.pk})
        )
    ...
    def test_should_create_new_object_geodetic_network_survey(self):
        response = client.post(
            reverse(
                "network-survey-create", kwargs={"pk": self.test_geodetic_work1.pk}
            ),
            data=self.data,
            follow=True,
        )
        ...
        self.assertEqual(
            GeodeticNetworkSurveyData.objects.filter(
                id_work=self.test_geodetic_work1.id
            ).count(),
            1,
        )
