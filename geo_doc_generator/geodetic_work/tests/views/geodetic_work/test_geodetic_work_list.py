...
class TestGeodeticWorkListView(TestCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_should_return_status_code_200_when_request_is_sent(self):
        ...
        resp = client.get(reverse("geodetic-work-list"))
    ...
    def test_should_return_correct_objects_when_request_is_sent(self):
        ...
        response = self.client.get(reverse("geodetic-work-list"))
        list_objects_works = response.context["works"]

        self.assertEqual(
            list(list_objects_works),
            [self.test_geodetic_work2, self.test_geodetic_work1],
        )
        self.assertTemplateUsed(response, "geodetic_work/geodetic_work_list.html")
